...
const register = async ({ email, name, phone, password }) => {
  try {
    ...
    // EmailVerification 레코드 생성 (Prisma/PostgreSQL 사용)
...
const completeRegistration = async ({ email, name, phone, password }) => {
  try {
    // 인증 완료 여부 확인 (Prisma/PostgreSQL)
    const verification = await prisma.emailVerification.findFirst({
      where: {
        email,
        verified: true
